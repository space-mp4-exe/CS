...
# This file was *autogenerated* from the file findIrreducable.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_60 = Integer(60); _sage_const_0 = Integer(0); _sage_const_4 = Integer(4); _sage_const_2 = Integer(2); _sage_const_283 = Integer(283)#from sage.rings.integer_classes import is_prime_power
...
def find_irreducible(n, m, primitive=False, verbose=False):
    ...
    field_size  = n**m
    group_order = field_size - _sage_const_1 
    ...
    def is_primitive(f):
        ...
        for q in prime_factors:
            if pow(x, group_order // q, f) == _sage_const_1 :
                return False
        return True
    ...
    candidates_tried = _sage_const_0 

    def check(f):
        nonlocal candidates_tried
        candidates_tried += _sage_const_1 
        if primitive:
    ...
    for k in range(_sage_const_1 , m):
        f = x**m + x**k + F(_sage_const_1 )
        if check(f):
            if verbose:
    ...
    if m >= _sage_const_4 :
        for a in range(_sage_const_1 , m):
            for b in range(_sage_const_1 , a):
                for c in range(_sage_const_1 , b):
                    f = x**m + x**a + x**b + x**c + F(_sage_const_1 )
                    if check(f):
                        if verbose:
    ...
    import random
    while True:
        coeffs = [F.random_element() for _ in range(m)] + [F(_sage_const_1 )]
        f = R(coeffs)
        if check(f):
            if verbose:
