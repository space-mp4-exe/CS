...
void Traverse(Node* p, string s) {
	if(s == "pre"){
		...
		q.push(p);
		while(!q.empty()){
			if(q.front()->Get_left()){//apparently, if a pointer isn't null it counts it as true
...
Node* Convert2Array(Node* root, int height) {
	int arraysize = pow(2, height + 1) - 1;//number of possible nodes in the tree
	Node *result = new Node[arraysize];
	result[0].copy(root);
	int sizeMinusOne = pow(2, height) - 1;//number of nodes in the tree minus the possible number of nodes in the last tier
	for(int i = 0; i < sizeMinusOne; i++){
		if(result[i].Get_key() != 0){
			result[2*i+1].copy(result[i].Get_left());
			result[2*i+2].copy(result[i].Get_right());
		}
	}
	return result;
}
